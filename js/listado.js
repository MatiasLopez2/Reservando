...
Listado.prototype.buscarRestaurante = function(id) {
    var found = this.restaurantes.find(elem => elem.id == id);
    if(found){ 
        return found;
    }
    else{
...
var eliminaRepetidos = ((elem,index,self) => index === self.indexOf(elem));
...
Listado.prototype.obtenerCiudad = function() {
    ...
    ciudades = this.restaurantes.map(elem => elem.ubicacion);
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ciudadesNuevo = ciudades.filter(eliminaRepetidos);
    
    return ciudadesNuevo.sort();
}
...
Listado.prototype.obtenerRubro = function() {
    var rubroOriginal = [];
    //Se recorre el array de restaurantes y se va agregando al array creado, todas las ubicaciones o ciudades encontradas
    rubroOriginal = this.restaurantes.map(elem => elem.rubro);    
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var rubroNuevo = rubroOriginal.filter(eliminaRepetidos);

    return rubroNuevo.sort();
}
...
Listado.prototype.obtenerHorario = function() {
    ...
    arregloHorario = this.restaurantes.map(elem => elem.horarios);
    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarios = [];
    arregloHorario.forEach(function(elemento) {
        elemento.forEach(function(hora) {
            horarios.push(hora)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horarioNuevo = horarios.filter(eliminaRepetidos);

    return horarioNuevo.sort();
}
